...
[ -z $(grep "CONFIG_KERNEL_BUILD_USER=" y1.config) ] &&
    echo 'CONFIG_KERNEL_BUILD_USER="moekoritsu"' >>y1.config ||
    sed -i 's@\(CONFIG_KERNEL_BUILD_USER=\).*@\1$"moekoritsu"@' y1.config

# Add kernel build domain
[ -z $(grep "CONFIG_KERNEL_BUILD_DOMAIN=" y1.config) ] &&
    echo 'CONFIG_KERNEL_BUILD_DOMAIN="GitHub Actions"' >>y1.config ||
    sed -i 's@\(CONFIG_KERNEL_BUILD_DOMAIN=\).*@\1$"GitHub Actions"@' y1.config
...
git clone https://github.com/jasonandy999/luci-app-frpc package/luci-app-app-frpc
...
svn co https://github.com/jasonandy999/bf-package-master/trunk/lean/luci-app-accesscontrol package/luci-app-accesscontrol #上网时间控制
